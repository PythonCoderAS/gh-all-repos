  ...
  async processList(options: ListOptions, repos: FullRepoData) {
    ...
    const filteredRepos = repos.filter((repo) => {
      if (
        ...
        repo.archived !== trueOptions.archived
      ) {
        return false;
      }

      if (
        trueOptions.private !== undefined &&
        repo.private !== trueOptions.private
      ) {
        return false;
      }
      ...
      if (
        trueOptions.template !== undefined &&
        repo.is_template !== trueOptions.template
      ) {
        return false;
      }
...
cli
  .command("list <user>", "List all repositories under an user/organization.")
  ...
  .option(
    "--languages",
    "Filters for repositories whose primary language is one of the supplied languages."
  )
  .option(
    "--no-show-owner-name",
    "Displays repository names only instead of 'owner/repo'."
  )
